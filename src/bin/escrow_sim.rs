...
impl Agent {
    ...
    async fn generate_and_send(&mut self, to: &str, extra_context: &str) -> anyhow::Result<()> {
        let system_prompt = format!(
            "You are {}. Role: {}. {}. Keep emails short, urgent, and slightly chaotic.",
            self.name, self.role, extra_context
        );
        ...
        let prompt = format!(
            "{}\n\nLast Messages:\n{:?}\n\nWrite a response to {}.",
            system_prompt, history_slice, to
        );
    ...
    async fn initiate(&mut self, to: &str, topic: &str) -> anyhow::Result<()> {
        self.history
            .push(json!({"role": "system", "content": format!("Event: {}", topic)}));
...
async fn main() -> anyhow::Result<()> {
    ...
    for turn in 1..8 {
        ...
        for i in indices {
            match i {
                0 => {
                    let _ = alice.check_inbox().await;
                }
                1 => {
                    let _ = bob.check_inbox().await;
                }
                2 => {
                    let _ = eddie.check_inbox().await;
                }
                _ => {}
            }
        ...
        if turn == 3 {
            ...
            eddie
                .history
                .push(json!({"role": "system", "content": "ALERT: FedWire delays reported."}));
            eddie
                .generate_and_send("Alice", "Massive system failure context")
                .await?;
            eddie
                .generate_and_send("Bob", "Massive system failure context")
                .await?;
        }
    }
